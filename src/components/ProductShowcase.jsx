...
export default function ProductShowcase({ product, onAdd }) {
  const price = product?.price ?? 12.9
  const title = product?.title ?? 'Aronia Pure - 100% Chokeberry Juice'
  const desc = product?.description ?? 'Cold-pressed from fresh aronia berries, unfiltered to retain natural goodness. Tart, rich, and invigorating.'
  const image = product?.image_url ?? 'https://images.unsplash.com/photo-1626595424320-b872d5efaa11?ixid=M3w3OTkxMTl8MHwxfHNlYXJjaHwxfHxDaG9rZWJlcnJ5JTIwSnVpY2UlMjBCb3R0bGV8ZW58MHwwfHx8MTc2MzU3OTA2MXww&ixlib=rb-4.1.0&w=1600&auto=format&fit=crop&q=80'
  ...
  return (
    <section id="product" className="relative py-24 bg-gradient-to-b from-slate-950 to-slate-900">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-10 items-center">
        ...
        >
          <img src={image} alt="Chokeberry Juice Bottle" className="w-full h-[420px] object-cover" />
        </motion.div>
        ...
        >
          ...
          <div className="mt-8 flex items-end gap-6">
            <div>
              <div className="text-4xl font-extrabold text-white">${Number(price).toFixed(2)}</div>
              <p className="text-xs text-slate-400">incl. VAT</p>
            </div>
            <button onClick={onAdd} className="inline-flex items-center gap-2 bg-fuchsia-500 hover:bg-fuchsia-600 text-white font-semibold px-6 py-3 rounded-md shadow-lg shadow-fuchsia-500/30 transition-colors">
